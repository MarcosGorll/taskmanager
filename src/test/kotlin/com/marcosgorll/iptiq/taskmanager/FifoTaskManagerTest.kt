...
class FifoTaskManagerTest {
    ...
    @Test
    fun `when Creating FifoTaskManager given Capacity 0 then throw IllegalArgumentException`() {
        val exception = Assertions.assertThrows(IllegalArgumentException::class.java) {
            newFifoTaskManager(0)
        }
        Assertions.assertEquals("TaskManager Capacity has to be bigger than 0. It was 0", exception.message)
    }

    @Test
    fun `when adding Process given Reaching Maximum Capacity then Add New Process and Remove Old`() {
        val (taskManager, _) = newFifoTaskManager(1, Priority.LOW)
    ...
    @Test
    fun `when adding Process Lower Priority given Reaching Maximum Capacity then Add Process`() {
        val (taskManager, _) = newFifoTaskManager(1)
        val toBeKilledProcess = TestProcess(Process(Priority.HIGH))
        taskManager.addProcess(toBeKilledProcess)
        ...
        Assertions.assertEquals(process.getPid(), processesAfterAdd[0].getPid())
        Assertions.assertTrue(toBeKilledProcess.wasKilled())
    }

    @Test
    fun `when adding Process Same Priority given Reaching Maximum Capacity then Not Add Process`() {
        val (taskManager, _) = newFifoTaskManager(1, Priority.HIGH)
        val process = Process(Priority.HIGH)
    ...
    fun `when adding Process given Reaching Enough Capacity then Add Process`() {
        val (taskManager, _) = newFifoTaskManager(2, Priority.LOW)
        val process = Process(Priority.HIGH)
        taskManager.addProcess(process)
        val processes = taskManager.listRunningProcesses(OrderDefinition.BY_PID_ASC)
    ...
    fun `when list Process by PID ASC given List of Processes then list Return List with Process by PID ASC`() {
        val (taskManager, processes) = newFifoTaskManager(5,
    ...
    fun `when list Process by CREATION ASC given List of Processes then list Return List with Process by CREATION ASC`() {
        val (taskManager, processes) = newFifoTaskManager(
            5,
    ...
    fun `when kill Process by PID given PID does exist then Kill the Process`() {
        val (taskManager, builtProcesses) = newFifoTaskManager(2, Priority.LOW, Priority.LOW)
        val processes = taskManager.listRunningProcesses(OrderDefinition.BY_PID_ASC)
    ...
    fun `when kill Process by PRIORITY given PRIORIY does exist then Kill the Process`() {
        val (taskManager, builtProcesses) = newFifoTaskManager(2,
    ...
    fun `when kill Process by PRIORITY given PRIORITY does not exist then does not Kill the Process`() {
        val (taskManager, _) = newFifoTaskManager(2, Priority.LOW, Priority.LOW)
        val processes = taskManager.listRunningProcesses(OrderDefinition.BY_CREATION_TIME_ASC)
